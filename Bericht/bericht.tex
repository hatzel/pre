...
This report aims to compare some of the known compression algorithms in both speed of decompression and compression as well as the compression ratio. We specifically looked at scientific data as it is prevalent in HPC application.
Overall we saw that, while a lot of scientific data already has compression built into the used file format, a lot of memory can be saved on uncompressed data.
Depending on whether speed or compression ratio are more important an a specific case different algorithms might be the right choice.
\thispagestyle{empty}
...
All these results are summed up in Figure \ref{fig:all_comp} (values in Table \ref{tab:all_comp})
...
For archival one has to make the trade off of memory and computing power. Depending on how both factors are valued one might come to the conclusion that bzip is in fact the best algorithm. Other papers have found lz4 to have a good trade off even for long term archival. \cite{PPerfHPC}
