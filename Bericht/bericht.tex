...
This can be attributed to the fact that the algorithms have a smaller 'backlog' of data to look for possible compression and therefore operate faster. \mcite{ResComp}
...
The GRIB2 files appear to also be compressed. GRIB2 does support, file format specific, compression \mcite{GRIMP}.

\FloatBarrier
Joined all this results lead to the Figure \ref{fig:all_comp} with values in Table \ref{tab:all_comp}.
Furthermore you can look at the joinde decompression results in Figure \ref{fig:all_decomp} with their values in Table \ref{tab:all_dec}.
...
On the other hand other papers have found lz4 to have the better trade off even for long term archival\mcite{PPerfHPC}.
\\
