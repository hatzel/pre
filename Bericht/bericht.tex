...
lz4 and lzo are both based on the lz77 algorithm, the differences are mostly in the variability and implementation of the algorithms and might be limited.
...
The optimum way would also prefer the longest found match.
As a result these algorithms work by iterating once over the data. 
...
\subsubsection{BZIP2}
In contrast bzip2 works with big compression stack resulting in multiple iterations over the data. 
The stack contains huffman codes, move-to-front transformation, Burrows–Wheeler transform and Delta encoding. 
Some of those are applied mutliple times. 
As a result bzip2 is expected to have very long runtimes.
The suspected upside however are great compression ratios since multiple passes might be able to find additional redundancy in the data.
...
Surprisingly the compression speeds of bzip2 and zlib are similar, though bzip2 consits of far more compression layers.
This can be attributed to multiple Huffman coding.
